...
public:
  double findMedianSortedArrays(vector<int>& nums1, vector<int>& nums2) {
    ...
    if (length & 1)
      return findKth(vector<int>& nums1, vector<int>& nums2, 0, 0, length/2 + 1);
    else
      return (findKth(vector<int>& nums1, vector<int>& nums2, 0, 0, length/2) +
              findKth(vector<int>& nums1, vector<int>& nums2, 0, 0, length/2 + 1)) / 2.0;
  }
